...
const defaultGet = async <T>(endpoint: string): Promise<T> => {
  const { data } = await axios.get(endpoint);
  return data;
};

const defaultPost = async <T>(endpoint: string, body: T): Promise<T> => {
  const { data } = await axios.post(endpoint, body);
  return data;
};

const defaultPut = async <T>(endpoint: string, body: T): Promise<T> => {
  const { data } = await axios.put(endpoint, body);
  return data;
};

const defaultDelete = async <T>(endpoint: string): Promise<T> => {
